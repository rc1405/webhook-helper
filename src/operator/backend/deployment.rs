...
use super::{validate_container_name, convert_to_deployment, convert_to_pod};
        ...
        if let Ok(mut d) = convert_to_deployment(self.webhook.spec.deployment.clone()).await {    
            ...
            match d.spec.clone() {
                Some(mut deployment_spec) => {
                    validate_container_name(self.webhook.spec.container_name.clone(), deployment_spec.template.spec.clone()).await?;
                ...
                None => return Err(Error::UnableToCreateObject("No DeploymentSpec found in WebhookHelper".into())),
            };
            ...
            validate_container_name(self.webhook.spec.container_name.clone(), p.spec.clone()).await?;
            ...
            return Err(Error::UnableToCreateObject("Invalid deployment spec".into()));
        }
    }
        ...
        if let Some(deployment) = self.deployment.clone() {
            match deployment {
                DeploymentType::Deployment(dep) => {
                    let result = perform_operation(self.client.clone(), Operation::Get, &dep).await?;
                ...
                DeploymentType::Pod(pod) => {
                    let result = perform_operation(self.client.clone(), Operation::Get, &pod).await?;
                    if let Some(status) = result.status.clone() {
                        if let Some(container_status) = status.conditions {
                            let statuses: Vec<String> = container_status.iter().filter(|s| s.type_ == "Ready").map(|s| s.status.clone()).collect();
                            if statuses.iter().filter(|n| *n == &"True".to_string()).count() > 0 {
