...
use super::{Operation, update_status, determine_stage, perform_cluster_operation, perform_cluster_get, perform_get};
use crate::crd::{WebhookHelper, Stage, WebhookType};
...
impl WebhookStage {
    pub fn new(client: Client, operation: Operation, webhook: WebhookHelper, service: Option<Service>) -> WebhookStage {
        WebhookStage { 
            client, 
            operation,
            webhook,
            ...
            Operation::Delete => {
                if let Some(status) = self.webhook.status.clone() {
                    if let Some(webhook) = status.validating_webhook {
                        let hook: ValidatingWebhookConfiguration = perform_cluster_get(self.client.clone(), &webhook).await?;
                        self.webhook_resource = Some(WebhookType::Validating(hook));
                    } else if let Some(webhook) = status.mutating_webhook {
                        let hook: MutatingWebhookConfiguration = perform_cluster_get(self.client.clone(), &webhook).await?;
                        ...
                        if let Some(uid) = self.webhook.uid() {
                            match webhook {
                                WebhookType::Mutating(d) => {
                                    perform_cluster_operation(self.client.clone(), Operation::ApplyOwner(uid), &d).await?;
                                },
                                WebhookType::Validating(p) => {
                                    perform_cluster_operation(self.client.clone(), Operation::ApplyOwner(uid), &p).await?;
                                },
                            };
                        };
                    },
    ...
    async fn create_webhook(&mut self) -> Result<WebhookType, Error> {
        let config_map: ConfigMap = perform_get(self.client.clone(), "kube-root-ca.crt", "default").await?;
        let cluster_ca_string = match config_map.data {
            Some(cm) => {
                match cm.get("ca.crt") {
                    Some(c) => c.clone(),
                    None => return Err(Error::UnableToCreateObject("Cound not find signing CA".into())),
                }
            },
            None => return Err(Error::UnableToCreateObject("Cound not find signing CA".into())),
        };
        ...
        if let Some(service) = self.service.clone() {
            if let Ok(mut hook) = convert_to_mutating_webhook(self.webhook.spec.webhook.clone()).await {
                let current_webhooks = hook.webhooks.unwrap_or(Vec::new()).iter().map(|w|{
                    let mut nwh = w.clone();
                    nwh.client_config.url = None;
                    nwh.client_config.ca_bundle = Some(ByteString(cluster_ca_string.as_bytes().into()));
                ...
                let result = perform_cluster_operation(self.client.clone(), Operation::Create, &hook).await?;
                self.webhook_resource = Some(WebhookType::Mutating(result.clone()));
                Ok(WebhookType::Mutating(result))
        
            } else if let Ok(mut hook) = convert_to_admission_webhook(self.webhook.spec.webhook.clone()).await {
                let current_webhooks = hook.webhooks.unwrap_or(Vec::new()).iter().map(|w|{
                    let mut nwh = w.clone();
                    nwh.client_config.url = None;
                    nwh.client_config.ca_bundle = Some(ByteString(cluster_ca_string.as_bytes().into()));
...
async fn convert_to_admission_webhook(data: Value) -> Result<ValidatingWebhookConfiguration, Error> {
